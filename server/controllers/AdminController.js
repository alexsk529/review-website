import { Author, Comments, db, Like, Rate, Review, ReviewTag } from '../db.js';
...
class AdminController {
    ...
    async getAuthors() {
        let authors = await Author.findAll({
            attributes: ['email', 'author_name', 'created_at', 'last_login', 'role', 'status'],
    ...
    async deleteAuthor(emails) {
        ...
        await Promise.all(emails.map(async email => {
            await Rate.destroy({where: {email: email}});
            await Like.destroy({where: {email: email}});
            await Like.destroy({where: {recipient: email}});
            await Comments.destroy({where: {email: email}});
        }));
